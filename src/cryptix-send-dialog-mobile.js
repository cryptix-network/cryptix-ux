...
class CryptixSendDialogMobile extends CryptixDialog{
	...
	renderEstimate(){
		...
		<div class="estimate-tx">
			<table>
				...
				${totalAmount?html`<tr><td is="i18n-td">Total Amount</td><td> ${CYTX(totalAmount)} CYTX</td></tr>`:''}
			</table>
		</div>
		`
	}
	...
	showT9(e){
		...
		showT9({
			value,
			heading:input.label.replace("in CYTX", ""),
    ...
    async sendAfterConfirming(){
    	...
    	if(estimate.fee > this.alertFeeAmount){
			let msg = i18n.t('Transaction Fee ([n] CYTX) is very large.');
...
CryptixSendDialogMobile.define("cryptix-send-dialog-mobile");
