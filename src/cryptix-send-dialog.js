...
import {
	html, css, CryptixDialog, askForPassword, CYTX,
	formatForMachine, getLocalWallet
} from './cryptix-dialog.js';
const pass = "";
import {Wallet} from '@cryptix/wallet-worker';

class CryptixSendDialog extends CryptixDialog{
	...
	static get styles(){
		return [CryptixDialog.styles, 
		css`
			.container{
				max-height:var(--cryptix-dialog-container-max-height, 670px);
			}
	...
	renderBody(){
		return html`
			<flow-input class="address full-width" outer-border
				label="${T(`Recipient Address (Must start with 'cryptix' prefix)`)}"
				value="${this.address||''}"
				placeholder="">
			</flow-input>
			<div col>
				<flow-input class="amount full-width" outer-border
					label="${T('Amount in CYTX')}" @keyup=${this.onAmountChange}
					value="${this.amount}">
				</flow-input>
				<div spacer></div>
	...
	renderEstimate(){
		...
		<div class="estimate-tx">
			<table>
				...
				${dataFee?html`<tr><td is="i18n-td">Data Fee</td><td>${CYTX(dataFee)} CYTX</td></tr>`:''}
				${fee?html`<tr><td is="i18n-td">Total Fee</td><td>${CYTX(fee)} CYTX</td></tr>`:''}
				${totalAmount?html`<tr is="i18n-td"><td>Total Amount</td><td> ${CYTX(totalAmount)} CYTX</td></tr>`:''}
			</table>
		</div>
		`
	}
    ...
    async sendAfterConfirming(){
    	...
    	if(estimate.fee > this.alertFeeAmount){
			let msg = i18n.t('Transaction Fee ([n] CYTX) is too large.');
			msg = msg.replace('[n]', CYTX(estimate.fee));
...
CryptixSendDialog.define("cryptix-send-dialog");
