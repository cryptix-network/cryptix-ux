import {
	html, css, CryptixDialog, CYTX, txListStyle,
	paginationStyle, buildPagination,
	renderPagination, T
} from './cryptix-dialog.js';

class CryptixTXDialog extends CryptixDialog{
	...
	static get styles(){
		return [CryptixDialog.styles, paginationStyle, txListStyle,
		css`
			:host{
				position:fixed;
			...
			.container{
				max-height:var(--cryptix-dialog-container-max-height, 90%);
				max-width:var(--cryptix-dialog-container-max-width, 90%)
			}
...
CryptixTXDialog.define("cryptix-tx-dialog");
