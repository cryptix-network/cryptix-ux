import {
	html, css, FlowFormat, CryptixWalletUI, dpc,
	baseUrl, CYTX, renderPagination, buildPagination, paginationStyle,
...
} from './cryptix-wallet-ui.js';
export {isMobile, dontInitiatedComponent};
export class CryptixWalletMobile extends CryptixWalletUI{
	...
	static get styles(){
		return [CryptixWalletUI.styles, paginationStyle, swipeableStyle, css`
			:host{
			...
			.header{
				...
				padding:var(--cryptix-wallet-header-padding, 5px 10px);
				margin:var(--cryptix-wallet-header-margin, 0px);
			}
			...
			.pagination-box{
				padding:var(--cryptix-pagination-box-padding, 10px 5px;);
			}
			...
			.tabs-container{
				...
				border-top:2px solid var(--cryptix-wallet-tab-border-top-color, var(--flow-primary-color));
				border-bottom:1px solid var(--cryptix-wallet-tab-border-bottom-color, #DDD);
			}
			.tabs{
			...
			.address-input{
				...
				min-width:var(--cryptix-wallet-address-input-min-width, 460px);
			...
			.send-scan-buttons a fa-icon{
				...
				background-color:var(--cryptix-wallet-scan-button-bg, #3d4e58);
				--fa-icon-color:var(--cryptix-wallet-scan-button-color, #FFF);
				box-shadow:var(--flow-box-shadow);
			}
			.send-scan-buttons .send-btn fa-icon{
				background-color:var(--cryptix-wallet-send-button-bg, #fb7470);
				--fa-icon-color:var(--cryptix-wallet-send-button-color, #FFF);
			}
			.send-scan-buttons .receive-btn fa-icon{
				background-color:var(--cryptix-wallet-receive-button-bg, #60b686);
				--fa-icon-color:var(--cryptix-wallet-receive-button-color, #FFF);
			}
			...
			.lock-screen{
				...
				position:absolute;
				z-index:1000000;
				background-color:var(--cryptix-lock-screen-bg-color);
				top:0px;bottom:0px;
				left:0px;right:0px;
			}
		...
		<div class="tab-contents flow-swipeable-container" ?not-ready=${!isReady}>
			<div class="flow-swipeable-row">
				...
				<div class="tab-content ${sCls('wallet')}" for="wallet">
					<div class="wallet-ux">
						<div class="badge" is="i18n-div">CRYPTIX WALLET</div>
						...
						<flow-expandable class="developer-info" _expand no-info no-icon icon="-">
							...
							<div class="badge"><span is="i18n-span">Cryptix Core CYTX:</span> ${window.PWA_MODULES['@cryptix/core-lib']}</div>
							<div class="badge"><span is="i18n-span">Cryptix Wallet Framework CYTX:</span> ${window.PWA_MODULES['@cryptix/wallet']}</div>
							<div class="badge"><span is="i18n-span">Cryptix gRPC:</span> ${window.PWA_MODULES['@cryptix/grpc']}</div>
							<div class="badge"><span is="i18n-span">Cryptix gRPC Relay:</span> ${window.PWA_MODULES['@cryptix/grpc-web']}</div>
							<div class="badge"><span is="i18n-span">Cryptix UX CYTX:</span> ${window.PWA_MODULES['@cryptix/ux']}</div>
							<div class="badge"><span is="i18n-span">Flow UX CYTX:</span> ${window.PWA_MODULES['@aspectron/flow-ux']}</div>
						</flow-expandable>
				...
				<div class="tab-content ${sCls('faucet')}" for="faucet">
					${this.faucetStatus ? this.faucetStatus : html`
						...
						<div class="faucet-ux">
							<div class="margin-bottom"  is="i18n-div">CRYPTIX FAUCET</div>
							<div>${i18nFormat('Your IP is [n]', this.ip||"")}</div>
							<div class="margin">${i18nHTMLFormat('You have <b>[n] CYTX</b> available.', CYTX(this.faucetFundsAvailable||0) )}</div>
				...
				<div class="tab-content ${sCls('debuginfo')}" for="debuginfo">
					<div class="info-ux">
						...
						<table>
							...
							<tr><td is="i18n-td">AMOUNT</td><td>${CYTX(inUseUTXOs.satoshis||0)} CYTX</td></tr>
						</table>
						<flow-btn class="center-btn primary v-margin"
	...
	renderLockScreen(){
		return html`
			<div class="lock-screen">
				<div><img class="big-logo" src="${baseUrl+'/resources/images/cryptix.png'}"></div>
	...
	openAddressExplorer(address){
		if (!address)
			return
		let url = `https://explorer.cryptix-network.org/addresses/${address}`;
  			...
  			<div class="balance-badge">
				${ this.isLoading ? html`
					...
					<div class="balance pending">
						<span class="label-pending">${T('PLEASE WAIT')} <span class="dots">${this.dots}</span> ${total ? this.formatCYTX(total)+' CYTX':''}</span>
					</div>
				` : html`
					<div class="balance">
						<span class="value">${this.formatCYTX(available)} CYTX</span>
					</div>
					<div class="balance pending">
						<span class="label-pending" is="i18n-span">Pending:</span>
						<span class="value-pending">${this.formatCYTX(pending)} CYTX</span>
					</div>
				`}
            </div>
	...
	showReceiveDialog(){
		let address = this.receiveAddress||'cryptixtest:abc'
