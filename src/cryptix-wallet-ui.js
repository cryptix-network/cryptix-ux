...
import {
	Deferred, GetTS, CYTX, formatForMachine, formatForHuman,
...
import {initCryptixFramework, Wallet, workerLog} from '@cryptix/wallet-worker';
export {Wallet};
Wallet.setWorkerLogLevel(localStorage.walletWorkerLogLevel || 'none')

export {html, css, FlowFormat, dpc, baseUrl, debug};

export class CryptixWalletUI extends BaseElement{
	...
	async initNetworkSettings() {
		...
		//const { network, port } = this.local_cryptixd_settings;
			...
			<div class="tx-notifications">
				${notifications.map(n=>{
					return html`<div class="tx-notification">
						${n.compoundUTXOs?
							T(`Compounding UTXOs...`):
							i18n.t(`Preparing transaction for [n] CYTX ....`)
							.replace('[n]', this.formatCYTX(n.amount))}
					</div>`
				})}
				...
				return html`
					<flow-expandable class="tx-row" static-icon expand ?txin=${tx.in} ?txmoved=${tx.isMoved} ?txout=${!tx.in}
						...
						<div class="tx-title" slot="title">
							...
							<div class="amount">
								${tx.in?'':'-'}${this.formatCYTX(tx.amount)} CYTX
							</div>
						</div>
						...
						<div class="tx-body">
							${tx.note}
							<div class="tx-id">
								<a target="_blank" href="https://explorer.cryptix-network.org/txs/${tx.id}">${tx.id}</a>
							</div>
							<div class="tx-address">
								${tx.myAddress?T('COMPOUNDING WALLET => '):''}
								<a target="_blank" href="https://explorer.cryptix-network.org/addresses/${tx.address}">${tx.address}</a>
							</div>
						</div>
					</flow-expandable>
				`
			...
			<div class="tx-list">
				${items.map((tx, i)=>{
					...
					<div class="tx-row" ?txin=${tx.in} tx-version=${tx.version} ?txmoved=${tx.isMoved} ?txout=${!tx.in}>
						...
						<div class="tx-amount">${tx.in?'':'-'}${CYTX(tx.amount)} CYTX</div>
						<div class="br tx-note">${tx.note}</div>
						<div class="br tx-id">
							<a target="_blank" href="https://explorer.cryptix-network.org/txs/${tx.id.split(":")[0]}">${tx.id.split(":")[0]}</a>
						</div>
						<div class="tx-address">
							${tx.myAddress?T('COMPOUNDING WALLET => '):''}
							<a target="_blank" href="https://explorer.cryptix-network.org/addresses/${tx.address}">${tx.address}</a>
		...
		items.map((tx, i)=>{
			...
			fields.forEach(field=>{
				switch(field){
					...
					break;
					case 'amount':
						row.push(escape(`${tx.in?'':'-'}${CYTX(tx.amount)}`))
					break;
					case 'direction':
			...
			<div class="tx-list">
				${items.map((tx, i)=>{
					...
					<div class="tx-row" ?iscoinbase=${!tx.isCoinbase}>
						...
						<div class="tx-amount">${CYTX(tx.satoshis)} CYTX</div>
						<div class="br tx-mass"></div>
						<div class="br tx-id">
							<a target="_blank" href="https://explorer.cryptix-network.org/txs/${tx.txId}">${tx.id}</a>
						</div>
						<div class="tx-address">
							<a target="_blank" href="https://explorer.cryptix-network.org/addresses/${tx.address}">${tx.address}</a>
						</div>
	...
	async alertUTXOIndexSupportIssue(){
		...
		let body = html`${i18nHTMLFormat(`'utxoindex' flag is missing from CRYPTIXD config.<br />
	...
	connectedCallback(){
		...
		let openDialog = document.createElement('cryptix-open-dialog');
		openDialog.hideLogo = !!this.hideOpenWalletLogo;
		this.parentNode.insertBefore(openDialog, this.nextSibling)
		this.sendDialog = document.createElement("cryptix-send-dialog"+mobileSuffix);
		this.parentNode.appendChild(this.sendDialog);
		this.receiveDialog = document.createElement("cryptix-receive-dialog"+mobileSuffix);
		this.parentNode.appendChild(this.receiveDialog);
		this.seedsDialog = document.createElement("cryptix-seeds-dialog");
		this.parentNode.appendChild(this.seedsDialog);
		let t9Dialog = document.createElement("cryptix-t9-dialog");
		this.parentNode.appendChild(t9Dialog);
		let qrscannerDialog = document.createElement("cryptix-qrscanner-dialog");
		qrscannerDialog.debug = this.debugscanner
		this.parentNode.appendChild(qrscannerDialog);
		let uploadFileDialog = document.createElement("cryptix-upload-file-dialog");
		this.parentNode.appendChild(uploadFileDialog);

		const {workerCorePath} = window.CryptixConfig||{}
		initCryptixFramework({
			workerPath: workerCorePath||"/cryptix-wallet-worker/worker.js?ident="+(window.CryptixConfig?.ident||"")
		}).then(()=>{
			let encryptedMnemonic = getLocalWallet()?.mnemonic;
	...
	async handleInitDialogCallback({dialog, password, seedPhrase, encryptedMnemonic}){
		...
		if(!rpc)
			return FlowDialog.alert(i18n.t("Error"), i18n.t("Cryptix Daemon config is missing."));
	...
	showTxDialog(){
		if(!this.txDialog){
			this.txDialog = document.createElement("cryptix-tx-dialog");
	...
	async isValidAddress(address){
		...
		if(window.mobileMode && prefix=="cryptixtest")
			return true;
	...
	*/
	...
	async getCryptixFromFaucet(amount) {
	...
	requestFaucetFunds() {
		...
		showT9({
			...
			inputLabel:i18n.t('Amount in CYTX')
		}, ({value:amount, dialog})=>{
			let sompis = formatForMachine(amount||0);
			if(sompis > this.faucetFundsAvailable){
				let msg = i18n.t(`You can't request more than [n] CYTX.`)
						.replace("[n]", CYTX(this.faucetFundsAvailable||0))
				return dialog.setError(msg);//'
			}
			
			dialog.hide();

			this.getCryptixFromFaucet(amount)
