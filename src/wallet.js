...
import {helper, Storage, CONFIRMATION_COUNT, COINBASE_CFM_COUNT} from '@cryptix/wallet-worker';
export const {Deferred, CYTX, Decimal} = helper;
export {CONFIRMATION_COUNT, COINBASE_CFM_COUNT};
const storage = new Storage({logLevel:'debug'});
let {baseUrl, debug, MAX_UTXOS_THRESHOLD=80, dontInitiatedComponent=false} = window.CryptixConfig || {};
if(!baseUrl){
	baseUrl = (new URL("../", import.meta.url)).href;
	debug && console.log("CryptixUX: baseUrl", baseUrl)
}
...
/*
const {Wallet, initCryptixFramework, Storage} = require("cryptix-wallet-worker");
let {Mnemonic} = Wallet;
...
export const {RPC} = require("cryptix-grpc-node");
*/
